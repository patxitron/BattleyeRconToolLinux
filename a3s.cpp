...
a3s::a3s(boost::asio::io_service& io_service, uint16_t port, char const* addr,
         on_data_func on_data, on_login_func on_login)
		:loged_in_(false)
		,listening_(false)
		,sequence_(0)
		,on_data_(on_data)
...
void a3s::login(const char* password)
{
	if(!listening_) {
		send_request(password, strlen(password), LOGIN);
		listening_ = true;
		receive();
	}
}
...
void a3s::receive()
{
	socket_.async_receive_from(
		boost::asio::buffer(data_, MSG_MAX_LENGTH), endpoint_,
		[this](boost::system::error_code ec, std::size_t length)
		{
			if(!ec && length > 0) {
				request_t* req = reinterpret_cast<request_t*>(data_);
				if(req->hdr.begin_header[0] == 'B'
							&& req->hdr.begin_header[1] == 'E'
							&& req->hdr.end_header == 0xff) {
					boost::crc_32_type crc;
					crc.process_bytes(data_ + 6, length - 6);
					if(req->hdr.checksum == crc.checksum()) {
						if(req->hdr.begin_msg == 0x00) {
							...
							if(req->payload[0] == 0x00 || req->payload[0] == 0x01) {
								...
								// Multipart packet NOT YET IMPLEMENTED
							}
